...
export default async function FacultyCourseOfferingPage(props: PageProps<"/faculty/courses/[offeringId]">) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link href="/faculty/courses" className="text-sm text-slate-500 underline hover:text-slate-900 dark:hover:text-slate-50">
          ← My Courses
        </Link>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Roster ({roster.length})</h2>
        {roster.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">No students enrolled in this offering yet.</p>
        ) : (
            ...
            <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
              <thead className="bg-slate-50 dark:bg-slate-900">
                <tr>
                  ...
                  <th className="whitespace-nowrap px-4 py-2.5 text-left font-medium text-slate-500 dark:text-slate-400">Enrollment</th>
                  <th className="whitespace-nowrap px-4 py-2.5 text-right font-medium text-slate-500 dark:text-slate-400">Attendance</th>
                  <th className="whitespace-nowrap px-4 py-2.5 text-left font-medium text-slate-500 dark:text-slate-400">Grade Status</th>
                </tr>
              </thead>
                  ...
                  <tr key={r.enrollmentId}>
                    ...
                    <td className="whitespace-nowrap px-4 py-2.5">
                      {r.result ? (
                        ...
                        <span className="text-xs text-slate-400 dark:text-slate-600">Not graded</span>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Sessions &amp; Attendance ({sessions.length})</h2>
        {sessions.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">No sessions scheduled yet.</p>
        ) : (
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Grades</h2>
