  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50">{milestone.title}</h1>
          </div>
      ...
      {milestone.description && (
        <p className="text-sm text-zinc-700 dark:text-zinc-300">{milestone.description}</p>
      )}
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Dates</h2>
        {historyEvents.length === 0 ? (
          <p className="text-sm text-zinc-500 dark:text-zinc-400">No dates recorded yet for this milestone.</p>
        ) : (
          <ol className="space-y-2">
            {historyEvents.map((e) => (
              ...
              >
                ...
                <span className="text-zinc-500 dark:text-zinc-400">{formatAcademicDate(e.date)}</span>
              </li>
            ))}
          </ol>
        )}
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Comments</h2>
        {milestone.record?.remarks ? (
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">No comments recorded yet.</p>
        )}
      </section>
      ...
      <section className="space-y-4">
        ...
        {requirementCards.length === 0 ? (
          <p className="text-sm text-zinc-500 dark:text-zinc-400">No documents are configured for this milestone.</p>
        ) : (
            ...
            {requirementCards.map(({ requirement: r, versionsWithUrls, latest, nextVersion, needsAction }) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="font-medium text-zinc-900 dark:text-zinc-50">{r.document_name}</p>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400">
                  ...
                  {latest ? (
                    ...
                    <span className="text-sm text-zinc-400 dark:text-zinc-600">Not submitted</span>
                  )}
                </div>
                ...
                {versionsWithUrls.length > 0 && (
                  <ul className="mt-3 space-y-1.5">
                    {versionsWithUrls.map(({ submission: v, url }) => (
                      <li
                        key={v.id}
                        className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-zinc-100 px-3 py-1.5 text-sm dark:border-zinc-900"
                      >
