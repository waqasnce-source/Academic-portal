  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">{thesis.thesis_title ?? "No title recorded yet"}</p>
      </div>

      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Thesis Record</h2>
        <ThesisEditForm
          thesisId={thesis.id}
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Reviewers &amp; Reviews ({reviewers.length})</h2>
        {reviewers.length > 0 && (
            ...
            {reviewers.map((r) => (
              <li key={r.id} className="rounded-lg border border-zinc-200 p-3 dark:border-zinc-800">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="font-medium text-zinc-900 dark:text-zinc-50">{r.reviewer_name}</p>
                ...
                {(reviewsByReviewer.get(r.id) ?? []).length > 0 && (
                  <ul className="mt-2 space-y-1.5">
                    {(reviewsByReviewer.get(r.id) ?? []).map((rv) => (
                      <li key={rv.id} className="rounded-md bg-zinc-50 px-3 py-1.5 text-sm dark:bg-zinc-900">
                        <span className="text-zinc-700 dark:text-zinc-300">
                          {formatAcademicDate(rv.received_date)} — {rv.recommendation ?? "No recommendation recorded"}
                        </span>
                        {rv.comments && <p className="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">{rv.comments}</p>}
                      </li>
                    ))}
                  </ul>
                )}
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Corrections ({corrections.length})</h2>
        {corrections.length > 0 && (
          <ul className="space-y-2">
            {corrections.map((c) => (
              <li key={c.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-zinc-200 px-3 py-2 text-sm dark:border-zinc-800">
                <span className="text-zinc-700 dark:text-zinc-300">
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Viva / Defence</h2>
        <VivaForm
          studentId={thesis.student_id}
          thesisId={thesis.id}
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Result Declaration</h2>
