...
export default async function ManagementDocumentReviewPage(
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/management/documents" className="text-sm text-slate-500 underline hover:text-slate-900 dark:hover:text-slate-50">
          ← Documents
        </Link>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">All Versions ({versions.length})</h2>
        <ul className="space-y-2">
          {versionsWithUrls.map(({ version: v, url }) => (
            <li
              key={v.id}
              className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-slate-200 px-4 py-2.5 text-sm dark:border-slate-800"
            >
              ...
              <div className="flex items-center gap-3">
                {v.remarks && <span className="text-xs text-slate-500 dark:text-slate-400">&quot;{v.remarks}&quot;</span>}
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="mt-1 text-sm capitalize text-slate-900 dark:text-slate-50">{value}</p>
    </div>
  );
}
